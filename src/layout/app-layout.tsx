...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiClock } from 'react-icons/fi';
...
import { getClocks } from 'apiSdk/clocks';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['viewer', 'scheduler'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ClockInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/clocks` : null),
    () => getClocks({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Clock',
      path: isTenantUser ? `/clocks/view/${data?.[0]?.id}` : '/clocks',
      entity: 'clock',
